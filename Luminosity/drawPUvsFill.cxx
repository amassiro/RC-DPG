...
//   
  ...
  std::vector<int> v_fill {
    8392, 8395, 8398, 8399, 8401, 8402, 8412, 8413, 8456, 8471, 8474, 8479, 8484, 8489, 8491, 8496, 8540, 8541, 8542, 8550, 8558, 8559, 8637, 8639, 8642, 8644, 8645, 8654, 8675, 8685, 8686, 8690, 8691, 8692, 8695, 8696, 8701, 8723, 8724, 8725, 8728, 8729, 8730, 8731, 8736, 8738, 8739, 8741, 8746, 8750, 8754, 8771, 8773, 8775, 8778, 8782, 8784, 8786, 8794, 8796, 8804, 8807, 8811, 8816, 8817, 8821, 8822, 8850, 8853, 8858, 8860, 8863, 8865, 8866, 8870, 8872, 8873, 8877, 8880, 8882, 8885, 8887, 8891, 8894, 8895, 8896, 8901, 8997, 8999, 9007, 9016, 9017, 9019, 9022, 9023, 9029, 9031, 9035, 9036, 9043, 9044, 9045, 9046, 9049, 9050, 9055, 9056, 9057, 9059, 9062, 9063, 9066, 9067, 9068, 9070, 9072, 9073
  };
...
//   };
  ...
  std::cout << " v_fill.size() = " << v_fill.size() << std::endl;
...
//     for (size_t i = 0; i < v_fill.size(); i++) {
    TString namefile = Form ("Luminosity/fill_%d.root", v_fill[i] );
//     fChain->Add(namefile.Data());
  ...
  TCanvas* canvas_all_gr = new TCanvas("canvas_all_gr", "avgpu vs time", 800, 600);
  
  TString toDraw = Form("avgpu:datetime");
//   fChain->Draw(toDraw, "1", "GOFF");
//   fChain->Draw(toDraw, "1", "");
//   fChain->Draw("avgpu", "1", "");
  ...
  TGraph* graph_maxavgpu = new TGraph(v_fill.size());
  TGraph* graph_minavgpu = new TGraph(v_fill.size());
  
  
  
  TCanvas* canvas_all = new TCanvas("canvas_all", "avgpu", 800, 600);
  canvas_all->Divide (10,5);
  ...
  for (size_t i = 0; i < v_fill.size(); i++) {
    ...
    TString namefile = Form ("Luminosity/fill_%d.root", v_fill[i] );
    TFile fileIn(namefile.Data(), "read");
    TTree* tree = (TTree*) fileIn.Get("newtree");
    
//     TString toDraw = Form("avgpu>>histoPU");
    TString toDraw = Form("avgpu>>histo_PU(1000, 0, 100)");
//     TString toDraw = Form("avgpu>>histo_PU(1000, 10, 100)");
    //     tree->Draw(toDraw.Data(), "1", "GOFF");
    tree->Draw(toDraw.Data(), "1", "");
    TH1F *histo_PU = (TH1F*)gPad->GetPrimitive("histo_PU"); // 1D
    
    std::cout << " toDraw :: " << toDraw.Data() << std::endl;
    std::cout << " tree :: " << tree->GetEntries() << std::endl;
    std::cout << " histo_PU :: " << histo_PU->GetEntries() << std::endl;
    std::cout << " fill[" << v_fill[i] << "] --> " << histo_PU->GetMean() << std::endl;
    std::cout << " ~~~ " << std::endl;
    
//     histo_PU->DrawClone("");
    histo_PU->Draw("");
    
    graph_avgpu->SetPoint(i, (map_fill_date[v_fill[i]].second).Convert(), histo_PU->GetMean());
    
    int binmax = histo_PU->GetMaximumBin(); 
    double xmax = histo_PU->GetXaxis()->GetBinCenter(binmax);
    graph_maxavgpu->SetPoint(i, (map_fill_date[v_fill[i]].second).Convert(), xmax);
    
    int binmin = histo_PU->GetMinimumBin(); 
    double xmin = histo_PU->GetXaxis()->GetBinCenter(binmin);
    graph_minavgpu->SetPoint(i, (map_fill_date[v_fill[i]].second).Convert(), xmin);
  }
...
//   graph_avgpu->Draw("APL");
  ...
  graph_maxavgpu->SetMarkerSize(1);
  graph_maxavgpu->SetMarkerStyle(201);
  graph_maxavgpu->SetMarkerColor(kRed+3);
  graph_maxavgpu->SetLineColor(kRed+3);
  
  
  graph_minavgpu->SetMarkerSize(1);
  graph_minavgpu->SetMarkerStyle(202);
  graph_minavgpu->SetMarkerColor(kRed+2);
  graph_minavgpu->SetLineColor(kRed+2);
...
//   
  ...
  mg->Add(graph_avgpu);
  mg->Add(graph_maxavgpu);
  mg->Add(graph_minavgpu);
  mg->Draw("APL");
  
  mg->GetXaxis()->SetTimeDisplay(1);
  mg->GetXaxis()->SetTimeFormat("%d/%m");
  TGaxis::SetMaxDigits(4);  // Increase the maximum number of digits for better spacing
  mg->GetXaxis()->SetLabelOffset(0.015);  // Adjust the label offset if needed
  mg->GetXaxis()->LabelsOption("v");  // Vertically align the labels
